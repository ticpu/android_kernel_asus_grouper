				      ...
				      struct vm_area_struct *vma)
{
	...
	if (mm) {
		...
		if (vma && mm != proc->vma_vm_mm) {
			printk("%d: vma mm and task mm mismatch\n",
				proc->pid);
			vma = NULL;
		}
	}
	...
	if (vma == NULL) {
		printk("%d: binder_alloc_buf failed to map pages in userspace, no vma\n",
			proc->pid);
		goto err_no_vma;
	}
	...
	for (page_addr = start; page_addr < end; page_addr += PAGE_SIZE) {
		...
		*page = alloc_page(GFP_HIGHUSER);
		if (*page == NULL) {
			printk("%d: binder_alloc_buf failed for page at %pK\n",
				proc->pid, page_addr);
		...
		if (ret) {
			printk("%d: binder_alloc_buf failed to map page at %pK in kernel\n",
			       proc->pid, page_addr);
		...
		if (ret) {
			printk("%d: binder_alloc_buf failed to map page at %lx in userspace\n",
					      ...
					      int is_async)
{
	...
	if (proc->vma == NULL) {
		printk("%d: binder_alloc_buf, no vma\n",
		       proc->pid);
		return NULL;
	}
	...
	if (best_fit == NULL) {
		printk("%d: binder_alloc_buf size %zd failed, no address space\n",
			proc->pid, size);
		return NULL;
	}
	if (n == NULL) {
	...
	if (buffer_size != size) {
		...
		if (!new_buffer) {
			printk("%s: %d failed to alloc new buffer struct\n",
			       __func__, proc->pid);
			   ...
			   struct list_head *target_list)
{
	if (strong) {
		if (internal) {
			if (target_list == NULL &&
			    ...
			    !(node->proc &&
			      ...
			      node->has_strong_ref)) {
				printk("invalid inc strong node for %d\n",
					node->debug_id);
				return -EINVAL;
			}
		...
		if (!node->has_weak_ref && list_empty(&node->work.entry)) {
			if (target_list == NULL) {
				printk("invalid inc weak node for %d\n",
					node->debug_id);
				return -EINVAL;
			}
			list_add_tail(&node->work.entry, target_list);
	...
	while (1) {
		...
		if (target_thread) {
			...
			if (target_thread->return_error == BR_OK) {
				...
				printk("reply failed, target thread, %d:%d, has error code %d already\n",
					      ...
					      binder_size_t *failed_at)
{
	...
	for (offp = off_start; offp < off_end; offp++) {
		...
		if (object_size == 0) {
			printk("transaction release %d bad object at offset %lld, size %zd\n",
		...
		case BINDER_TYPE_WEAK_BINDER: {
			...
			if (node == NULL) {
				printk("transaction release %d bad node %016llx\n",
		...
		case BINDER_TYPE_WEAK_HANDLE: {
			...
			if (ref == NULL) {
				printk("transaction release %d bad handle %ld\n",
				 debug_id, fp->handle);
				break;
			}
			binder_debug(BINDER_DEBUG_TRANSACTION,
		...
		case BINDER_TYPE_PTR:
			...
			break;
		default:
			printk("transaction release %d bad object type %lx\n",
				debug_id, hdr->type);
			break;
		}
	}
			...
			binder_size_t *consumed)
{
	...
	while (ptr < end && thread->return_error == BR_OK) {
		...
		case BC_ATTEMPT_ACQUIRE:
			printk("BC_ATTEMPT_ACQUIRE not supported\n");
			return -EINVAL;
		case BC_ACQUIRE_RESULT:
			printk("BC_ACQUIRE_RESULT not supported\n");
			return -EINVAL;
...
static void binder_release_work(struct list_head *list)
{
	...
	while (!list_empty(list)) {
		...
		default:
			printk("unexpected work type, %d, not freed\n",
			       w->type);
			break;
		}
	}
	...
	case BINDER_SET_CONTEXT_MGR:
		...
		if (context->binder_context_mgr_uid != -1) {
			if (context->binder_context_mgr_uid != current->cred->euid) {
				printk("binder: BINDER_SET_CONTEXT_MGR bad uid %d != %d\n",
				       current->cred->euid,
...
err:
	...
	if (ret && ret != -ERESTARTSYS)
		printk("%d:%d ioctl %x %lx returned %d\n", proc->pid, current->pid, cmd, arg, ret);
	return ret;
}
...
static void binder_deferred_release(struct binder_proc *proc)
{
	...
	while ((n = rb_first(&proc->allocated_buffers))) {
		...
		if (t) {
			t->buffer = NULL;
			buffer->transaction = NULL;
			printk("release proc %d, transaction %d, not freed\n",
			       proc->pid, t->debug_id);
			/*BUG();*/
		}
